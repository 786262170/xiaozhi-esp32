...
#define MAX_EXTERNAL_CAPTURE_SAMPLES (16000 * 300 / 1000)
...
private:
    ...
    // A bounded post-AFE PCM handoff used by transports which own their encoder
    // (for example LiveKit WebRTC). The producer never blocks; stale realtime
    // samples are discarded when the consumer falls behind.
    std::mutex external_capture_mutex_;
    std::condition_variable external_capture_cv_;
    std::deque<int16_t> external_capture_queue_;
